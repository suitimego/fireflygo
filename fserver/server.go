...
func (this *Server) handleConnection(conn *net.TCPConn) {
	...
	fconn.SetProperty(fnet.FIREFLYGO_CONN_PROPERTY_NAME, this.Name)
	fconn.Start()
}
...
func (this *Server) Start() {
	...
	go func() {
		...
		logger.Info(fmt.Sprintf("start fireflygo server %s...", this.Name))
		for {
...
func (this *Server) Stop() {
	logger.Info("stop fireflygo server ", this.Name)
