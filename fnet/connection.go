...
const (
	FIREFLYGO_CONN_PROPERTY_CTIME = "fireflygo_ctime"
	FIREFLYGO_CONN_PROPERTY_NAME  = "fireflygo_tcpserver_name"
)
...
func NewConnection(conn *net.TCPConn, sessionId uint32, protoc iface.IServerProtocol) *Connection {
	...
	//set  connection time
	fconn.SetProperty(FIREFLYGO_CONN_PROPERTY_CTIME, time.Since(time.Now()))
	return fconn
}

func (this *Connection) Start() {
	//add to connectionmsg
	serverName, err := this.GetProperty(FIREFLYGO_CONN_PROPERTY_NAME)
	if err != nil {
