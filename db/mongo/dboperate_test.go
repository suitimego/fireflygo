...
func Test_DailDB(t *testing.T) {
	dbcfg := NewDbCfg("127.0.0.1", 27017, "fireflygodb", "", "")
	//dbcfg := NewDbCfg("127.0.0.1", 27017, "fireflygodb", "admin", "admin")
...
//go test -v github.com\suitimego\fireflygo\db\mongo -run ^Test_CommonOperate$
func Test_CommonOperate(t *testing.T) {
	...
	//------------------------------------------------------------------------
	err = dbo.Insert("test", bson.M{"username": "fireflygo", "pass": "pass1111"})
	if err != nil {
		dbo.CloseDB()
		t.Fatal(err)
		return
	...
	err = dbo.Insert("test", bson.M{"username": "fireflygo_0", "pass": "pass1111"})
	if err != nil {
		dbo.CloseDB()
		t.Fatal(err)
		return
	...
	err = dbo.DBFindOne("test", bson.M{"username": "fireflygo"}, func(a bson.M) error {
		if a != nil {
			t.Log(a)
			return nil
		} else {
	...
	docs := make([]bson.M, 0)
	for i := 0; i < 500; i++ {
		docs = append(docs, bson.M{"username": fmt.Sprintf("fireflygo_%d", i), "pass": "pass1111"})
	}
...
//go test -v github.com\suitimego\fireflygo\db\mongo -bench ^Benchmark_CommonOperate$
...
//go test -v github.com\suitimego\fireflygo\db\mongo -bench ^Benchmark_CommonOperatePP$
