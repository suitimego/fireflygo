...
func (this *TelnetProtocol) StartReadThread(fconn iface.Iconnection) {
	...
	fconn.GetConnection().Write([]byte(fmt.Sprintf("-------welcome to fireflygo telnet tool(node: %s)---------\r\n", utils.GlobalObject.Name)))
	for {
		...
		logger.Info(fmt.Sprintf("fireflygo telnet tool received: %s. ip: %s", line, this.getConnectionAddr(fconn)[0]))
