...
func (this *ClusterServer) StartClusterServer() {
	...
	if serverconf.NetPort > 0 {
		...
		if serverconf.Host != "" {
			this.NetServer = fserver.NewTcpServer("fireflygocluster_net_server", "tcp4", serverconf.Host, serverconf.NetPort,
	...
	if serverconf.RootPort > 0 {
		if serverconf.Host != "" {
			this.RootServer = fserver.NewTcpServer("fireflygocluster_root_server", "tcp4", serverconf.Host, serverconf.RootPort,
	...
	logger.Info("fireflygo cluster start success.")
	// close
	this.WaitSignal()
	...
	logger.Info("fireflygo cluster stoped.")
}
